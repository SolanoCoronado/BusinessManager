...
// Variante reutilizable que opera dentro de una transaccion ya abierta por el llamador
// (por ejemplo, al anular una factura: anular = revertir su asiento + cambiar su estado,
// ambas cosas deben ocurrir juntas o no ocurrir).
export async function reverseJournalEntryWithClient(
  client: DbClient,
  companyId: string,
  actorUserId: string,
  entryId: string,
  ...
  const created = await client.journalEntry.create({
    data: {
      companyId,
      date: new Date(),
      memo: original.memo ? `Reverso de: ${original.memo}` : `Reverso del asiento ${original.id}`,
      status: "posted",
      sourceType: "reversal",
...
export function reverseJournalEntry(companyId: string, actorUserId: string, entryId: string) {
  return prisma.$transaction((tx) =>
    reverseJournalEntryWithClient(tx, companyId, actorUserId, entryId),
  );
}
